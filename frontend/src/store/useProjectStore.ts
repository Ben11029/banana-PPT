...
interface ProjectState {
  ...
  // 项目操作
  initializeProject: (type: 'idea' | 'outline' | 'description', content: string, templateImage?: File, imageResolution?: '1K' | '2K' | '4K', imageAspectRatio?: string) => Promise<void>;
...
const debouncedUpdatePage = debounce(
  ...
  // 初始化项目
  initializeProject: async (type, content, templateImage, imageResolution, imageAspectRatio) => {
    ...
    try {
      ...
      if (imageResolution) {
        request.image_resolution = imageResolution;
      }
      if (imageAspectRatio) {
        request.image_aspect_ratio = imageAspectRatio;
      }
